...
- `swift run diff-view-demo --smoke-test`: passed using a real WKWebView with bundled resources. Observed ready, rendered, source text in DOM, and openFile callback from a line-number double-click.
...
Codans local GUI core cases were verified against component revision `5df58273a300e247491aeafede9d237e84b5d716` and passed: Changes scopes and Outgoing, file-type handling, refresh, current-file launching in Cursor at line 4, focus behavior, and error recovery. The host repository maintains the case record at `docs/user-tests/git-diff-viewer.md`.
...
`npm test` passes six protocol/diff cases; `npm run build` succeeds; five WebKit browser cases pass, including hidden chrome, edge-to-edge layout, both line-number event modes, errors, and returning to default full chrome. `swift test` passes six cases; `swift run diff-view-demo --smoke-test` passes with a real line-number event. Browser screenshots are generated at `test-results/native-unified.png` and `test-results/native-split.png`. Host GUI styling is verified separately in Codans.
